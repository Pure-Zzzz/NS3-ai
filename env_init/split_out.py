...
color_map = ListedColormap(['white', 'orange', 'blue', 'olive', 'pink', 'green'])
...
def test_single_image(model, image_path, transform):
    ...
    return y
def process(terrain_path, weather_path):
    # 用于测试单个图像的示例用法
    model = Unet(3, 6)
    ...
    gray_img = test_single_image(model, terrain_path, transform)
    fig, ax = plt.subplots()
    ax.imshow(gray_img, cmap=color_map)
